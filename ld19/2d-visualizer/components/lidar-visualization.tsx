...
export default function LidarVisualization({ points }: LidarVisualizationProps) {
  ...
  useEffect(() => {
    ...
    if (pointsObjectRef.current) {
      ...
      if (Array.isArray(pointsObjectRef.current.material)) {
        pointsObjectRef.current.material.forEach((material) => material.dispose())
      } else {
    ...
    points.forEach((point, index) => {
      ...
      const angleRad = (point.a * Math.PI) / 180

      // Convert polar to cartesian coordinates
      // Using distance as radius, angle for rotation around Y axis
      const x = point.d * Math.cos(angleRad)
      const z = point.d * Math.sin(angleRad)
      const y = 0 // Keep points on the ground plane
      ...
      const intensityNorm = point.i / 255
      const distanceNorm = Math.min(point.d / 1000, 1) // Normalize distance to 0-1
