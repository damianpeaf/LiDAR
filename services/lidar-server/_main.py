...
@app.route("/api/ld19-lidar", methods=["POST"])
def receive_ld19_data():
    try:
        ...
        total_points = len(points)
        print(f"Total de puntos recibidos: {total_points}")

        if r:
            # Recuperar datos existentes
            existing_data = r.get("ld19")
            if existing_data:
                existing_points = json.loads(existing_data)
            else:
                existing_points = []

            # Agregar los nuevos puntos al arreglo existente
            existing_points.extend(points)

            # Guardar de nuevo en Redis
            r.set("ld19", json.dumps(existing_points))
            r.incr("lidar_data_count")

            print("Puntos LD19 acumulados en Redis")
        else:
        ...
        print(f"Error procesando datos LD19: {e}")
        return "", 500
